...
class SavedInspection(Base):
    __tablename__ = "saved_inspections"
    ...
    vps_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("vps_instances.id"), nullable=False
    )
    ...
    vps: Mapped[VpsInstance] = relationship("VpsInstance")
